    ...
    <section className="py-32 relative overflow-hidden bg-[#1E1E1E] text-white">
      ...
      <div className="container relative z-10 text-center">
        ...
        <div className="relative h-40 md:h-64 flex items-center justify-center group max-w-5xl mx-auto">
          ...
          {/* Active Line Glow */}
          <div className="absolute top-1/2 left-0 right-0 h-[1px] bg-gradient-to-r from-transparent via-orange-500/20 to-transparent blur-sm" />

          {/* Content Container */}
          <div className="w-full relative z-10">

            {/* Animated Energy Flow - Orange Beam */}
            <div className="absolute left-0 right-0 top-1/2 h-1 -translate-y-1/2 pointer-events-none overflow-hidden">
              <motion.div
                className="absolute top-0 h-full w-32 bg-gradient-to-r from-transparent via-orange-500 to-transparent blur-[2px]"
                animate={{ left: ["-20%", "120%"] }}
            ...
            <div className="flex justify-between items-center relative px-4 md:px-12">
              {/* Web2 Side */}
              <motion.div
                className="text-left cursor-default p-6 rounded-2xl border border-white/5 bg-[#252525] hover:border-orange-500/30 transition-all duration-500"
                whileHover={{ x: 5 }}
              >
              ...
              <div className="relative z-20 mx-4">
                <motion.div
                  className="w-20 h-20 bg-[#1E1E1E] rounded-full border border-orange-500/50 flex items-center justify-center shadow-[0_0_30px_rgba(249,115,22,0.15)] relative"
                  animate={{
                    boxShadow: ["0 0 30px rgba(249,115,22,0.15)", "0 0 50px rgba(249,115,22,0.3)", "0 0 30px rgba(249,115,22,0.15)"]
                  }}
                  transition={{ duration: 4, repeat: Infinity, ease: "easeInOut" }}
                >
                  <div className="absolute inset-0 rounded-full border border-white/10" />
                  <span className="text-3xl text-orange-500 relative z-10">∞</span>
                </motion.div>
              </div>

              {/* Web3 Side */}
              <motion.div
                className="text-right cursor-default p-6 rounded-2xl border border-white/5 bg-[#252525] hover:border-orange-500/30 transition-all duration-500"
                whileHover={{ x: -5 }}
              >
